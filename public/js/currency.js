...
exchangeHandler = (e) => {
  if (
    ...
    (e.type === "keydown" &&
      e.key === "Enter" &&
      document.activeElement === exchangeInput)
  ) {
    ...
    .then((countryData) => {
      ...
      if (symbol.indexOf(",") > -1) {
        ...
        for (let i = 0; i < symArray.length; i++) {
          renderedSymbol =
            renderedSymbol + String.fromCharCode(parseInt(symArray[i], 16));
        }
...
const exchangeRate = async (countryOne, countryTwo) => {
  ...
  const response = await fetch(
    `http://localhost:3000/exchange/${searchCountryCode}/${exchangeCountryCode}`
  );
...
const currencyCode = async (country) => {
  const response = await fetch(
    `http://localhost:3000/currency/code/${country}`
  );
